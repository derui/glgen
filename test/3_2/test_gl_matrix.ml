[%%suite
 ...
 let move_matrix scale =
   M.set matrix ~row:0 ~col:3 ~v:(sin scale);
   M.set matrix ~row:1 ~col:3 ~v:(sin scale);
   matrix
         ...
         let rec loop counter scale =
           if counter = 0 then ()
             ...
             begin
               ...
               let m = move_matrix scale |> M.to_array |> Array.to_list |> Array.concat in
